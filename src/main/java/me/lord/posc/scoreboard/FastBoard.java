package me.lord.posc.scoreboard;
...
// TODO: Complete overhaul to the scoreboard system utilizing nms
...
public class FastBoard {
	 ...
	 *
	 * @param uuid the owner of the scoreboard
	 */
	...
	public void updateAll() {
		updateTitle(TextUtil.cs("&6&lPosc &7(" + TextUtil.format(Posc.onlinePlayers) + "/" + TextUtil.format(Bukkit.getMaxPlayers()) + ")"));
		updateLines(
				"",
				"&6Balance&7: &f" + TextUtil.formatMoney(DataManager.getPlayerData(getPlayer()).getBalance()),
				"&6Kills&7: &f" + TextUtil.format(getPlayer().getStatistic(Statistic.PLAYER_KILLS)),
				"&6Deaths&7: &f" + TextUtil.format(getPlayer().getStatistic(Statistic.DEATHS)),
				"",
	...
	public void updateConstant() {
		updateLine(5, "&6Ping&7: &f" + TextUtil.format(getPlayer().getPing()) + " ms");
		updateLine(6, "&6TPS&7: &f" + TextUtil.format(Bukkit.getTPS()[0]));
	}

	public void updateBalance() {
		updateLine(1, "&6Balance&7: &f" + TextUtil.formatMoney(DataManager.getPlayerData(getPlayer()).getBalance()));
	}

	public void updateKills() {
		updateLine(2, "&6Kills&7: &f" + TextUtil.format(getPlayer().getStatistic(Statistic.PLAYER_KILLS)));
	}

	public void updateDeaths() {
		updateLine(3, "&6Deaths&7: &f" + TextUtil.format(getPlayer().getStatistic(Statistic.DEATHS)));
	}
	...
	private void sendPacket(Object packet) throws Throwable {
		...
		if (getPlayer().isOnline()) {
			Object entityPlayer = PLAYER_GET_HANDLE.invoke(getPlayer());
